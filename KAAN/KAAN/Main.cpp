...
void ICGUI_main()
{
	static ICBYTES m;
	int color = 0x2e27ff;
	CreateImage(m, 150, 100, ICB_UINT);

	FRM = ICG_FrameMedium(5, 5, 160, 110);
	...
	m = 0;
	SetPixels(kaan, 20, 20, 0x33cccc, m);
	ICBYTES  p, q;
	CreateImage(p, 19, 27, ICB_UINT);
	SetPixels(kaan, 1, 1, 0x22aaaa, p);
	MagnifyX3(p, q);
	Paste(q, 40, 15, m);
	MagnifyX3(m, TV);
	SetPixels(kaan, 20, 20, 0xffff, TV);
	SetPixelsX2(kaan, 30, 150, 0xcccccc, TV);
	DisplayImage(FRM, TV);
}